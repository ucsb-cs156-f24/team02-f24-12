...
export default function ArticlesEditPage({ storybook = false }) {
  ...
  const onSuccess = (articles) => {
    toast(
      `Articles Updated - id: ${articles.id} title: ${articles.title} url: ${articles.url} explanation: ${articles.explanation} email: ${articles.email} dateAdded: ${articles.dateAdded}`,
    );
  };
