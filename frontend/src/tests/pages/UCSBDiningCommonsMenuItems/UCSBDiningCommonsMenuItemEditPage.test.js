...
describe("UCSBDiningCommonsMenuItemEditPage tests", () => {
  describe("when the backend doesn't return data", () => {
    ...
    test("renders header but table is not present", async () => {
      ...
      expect(screen.queryByTestId("Menu Item-name")).not.toBeInTheDocument();
      restoreConsole();
    });
  });
  ...
  describe("tests where backend is working normally", () => {
    ...
    test("Is populated with the data provided", async () => {
      ...
      expect(nameField).toHaveValue("DeLaGuerra");
      ...
      expect(mockToast).toBeCalledWith(
        "Menu Item Updated - id: 17 diningCommonsCode: CRL name: Carillo station: Dessert",
      );
    ...
    test("Changes when you click Update", async () => {
      ...
      fireEvent.change(diningCommonsCodeField, { target: { value: "CRL" } });
      fireEvent.change(nameField, {
        target: { value: "Carillo" },
      });
      fireEvent.change(stationField, { target: { value: "Dessert" } });
