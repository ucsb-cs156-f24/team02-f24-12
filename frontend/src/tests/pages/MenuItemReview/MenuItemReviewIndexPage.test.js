...
describe("MenuItemReview 'Index' Page tests", () => {
  ...
  test("renders three reviews correctly for regular user", async () => {
    ...
    const createReviewButton = screen.queryByText("Create Menu Item Review");
    expect(createReviewButton).not.toBeInTheDocument();

    const reviewerEmail = screen.getByText("b@ucsb.edu");
    expect(reviewerEmail).toBeInTheDocument();

    const comments = screen.getByText((content) =>
      content.includes("This is my second review"),
    );
